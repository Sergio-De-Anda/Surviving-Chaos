...
public class Weapon : MonoBehaviour
{
    ...
    public void Fire(Ray ray)
    {
        ...
        RaycastHit hit;
        Vector3 bSpawnPoint = cam.position;
        ...
        {
            HitEffects(hit);
            hit.collider.SendMessage("ApplyDamage", 25.0f, SendMessageOptions.DontRequireReceiver);
        }
        GunEffects();
	...
	void HitEffects(RaycastHit hit)
	{
        if (hit.collider.gameObject.isStatic)
        {
            if (weaponSettings.decal)
			{
				...
				Destroy(decal, Random.Range(5.0f, 10.0f));
			}
        }
    }
